...
#define LZX_NUM_SECONDARY_LEN \
  ((LZX_MAX_MATCH - LZX_MIN_MATCH + 1) - LZX_NUM_PRIMARY_LEN)
...
int lzx_decompress_block(LZX_DECODER_CONTEXT* context, const uint8_t* src,
                         uint32_t src_size, uint8_t* dest,
...
/* Decompress the input buffer block by block into the output buffer
 ...
 dest_size: Output buffer size; returns the output buffer size. if output buffer
 is pre-allocated, this should be the size of the pre-allocated buffer.
 decompressed_size: Returns the decompressed size.
 returns 0 on SUCCESS, otherwise LZX_ERROR */
int lzx_decompress(const uint8_t* src, uint32_t src_size, uint8_t** dest,
...
/* Compress the input buffer block by block into the output buffer
 ...
 returns 0 on SUCCESS, otherwise LZX_ERROR */
int lzx_compress(const uint8_t* src, uint32_t src_size, uint8_t** dest,
