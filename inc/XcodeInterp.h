...
typedef struct _LABEL {
  ...
  uint32_t references;  // how many times the label is jumped to ( xcodes that
                        // use this label )
  bool defined;  // if we have come across this label definition. ( the xcode
                 // that this label points to )
  char name[20];
} LABEL;
...
class XcodeInterp {
 public:
  ...
  uint32_t offset;       // offset from the start of the data to the end of the
                         // current XCODE (offset to the next XCODE)
