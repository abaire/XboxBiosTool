// XbTool.cpp : Implements command functions for the Xbox Bios Tool. Logic for
// each command is implemented here.
...
static const CMD_TBL cmd_tbl[] = {
    ...
    {"bld",
     CMD_BUILD_BIOS,
     {SW_BLDR_FILE, SW_KRNL_FILE, SW_KRNL_DATA_FILE, SW_INITTBL_FILE},
     {SW_NONE}},
    {"split", CMD_SPLIT_BIOS, {SW_IN_FILE}, {SW_IN_FILE}},
    {"combine",
     CMD_COMBINE_BIOS,
     {SW_NONE},
     {SW_BANK1_FILE, SW_BANK2_FILE, SW_BANK3_FILE, SW_BANK4_FILE}},
...
static const PARAM_TBL param_tbl[] = {
    ...
    {"dir", &params.working_directory_path, SW_WORKING_DIRECTORY,
     PARAM_TBL::STR},
...
int buildBios() {
  ...
  build_params.init_tbl =
      readFile(params.init_tbl_file, &build_params.init_tbl_size, 0);
  ...
  if (params.preldr_file != nullptr) {
    build_params.preldr =
        readFile(params.preldr_file, &build_params.preldr_size, 0);
  ...
  build_params.compressed_kernel =
      readFile(params.kernel_file, &build_params.kernel_size, 0);
  ...
  build_params.kernel_data =
      readFile(params.kernel_data_file, &build_params.kernel_data_size, 0);
  ...
  if (params.eeprom_key_file != nullptr) {
    ...
    build_params.eeprom_key =
        readFile(params.eeprom_key_file, nullptr, XB_KEY_SIZE);
  }

  // cert key
  if (params.cert_key_file != nullptr) {
    printf("Cert key file:\t\t%s\n", params.cert_key_file);
    build_params.cert_key =
        readFile(params.cert_key_file, nullptr, XB_KEY_SIZE);
  }
...
int extractBios() {
  ...
  if (bios.preldr.status < PRELDR_STATUS_NOT_FOUND) {
    ...
    if (filename == nullptr) filename = "preldr.bin";
    writeFileF(filename, "preldr", bios.preldr.data, PRELDR_SIZE);

    // zero preldr
    memset(bios.preldr.data, 0, PRELDR_SIZE);
    // zero preldr params.
    memset(bios.preldr.data + PRELDR_SIZE + ROM_DIGEST_SIZE, 0,
           PRELDR_PARAMS_SIZE - sizeof(BOOT_PARAMS));
  }

  // 2bl
  filename = params.bldr_file;
  if (filename == nullptr) filename = "bldr.bin";
  ...
  if (init_tbl_size > 0) {
    ...
    if (filename == nullptr) filename = "inittbl.bin";
    writeFileF(filename, "init table", bios.data, init_tbl_size);
  }

  // extract compressed kernel
  filename = params.kernel_file;
  if (filename == nullptr) filename = "krnl.bin";
  writeFileF(filename, "compressed kernel", bios.kernel.compressed_kernel_ptr,
             bios.bldr.boot_params->compressed_kernel_size);

  // extract uncompressed kernel section data
  filename = params.kernel_data_file;
  if (filename == nullptr) filename = "krnl_data.bin";
  writeFileF(filename, "kernel data", bios.kernel.uncompressed_data_ptr,
             bios.bldr.boot_params->uncompressed_kernel_data_size);
  ...
  if (bios.decompressKrnl() == 0) {
    ...
    if (bios.kernel.img != nullptr) {
      writeFileF("krnl.img", "decompressed kernel", bios.kernel.img,
  ...
  if (isFlagSet(SW_KEYS)) {
    ...
    if (bios.bldr.keys != nullptr) {
      ...
      if (filename == nullptr) filename = "eeprom_key.bin";
      writeFileF(filename, "eeprom key", bios.bldr.keys->eeprom_key,
                 XB_KEY_SIZE);

      // cert rc4 key
      filename = params.cert_key_file;
      if (filename == nullptr) filename = "cert_key.bin";
      writeFileF(filename, "cert key", bios.bldr.keys->cert_key, XB_KEY_SIZE);

      // kernel rc4 key
      writeFileF("krnl_key.bin", "kernel key", bios.bldr.keys->kernel_key,
                 XB_KEY_SIZE);
    }
    ...
    if (params.mcpx.sbkey != nullptr) {
      writeFileF("sb_key.bin", "secret boot key", params.mcpx.sbkey,
                 XB_KEY_SIZE);
    }
    ...
    if (bios.kernel.img != nullptr) {
      ...
      if (filename == nullptr) filename = "pubkey.bin";
      if (rsa_findPublicKey(bios.kernel.img, bios.kernel.img_size, &pubkey,
                            nullptr) == RSA_ERROR_SUCCESS)
        writeFileF(filename, "public key", pubkey,
                   RSA_PUBKEY_SIZE(&pubkey->header));
    }

    // preldr; extract preldr components.
    if (bios.preldr.status < PRELDR_STATUS_NOT_FOUND) {
      writeFileF("preldr_key.bin", "preldr key", bios.preldr.bldr_key,
                 SHA1_DIGEST_LEN);
    }
  }
...
int splitBios() {
  ...
  if (params.romsize < MIN_BIOS_SIZE) return 1;
  ...
  bankFnLen = strlen(biosFn) + strlen(suffix) + strlen(ext) +
              3U;  // +3 for the bank number
  ...
  while (dataLeft > 0) {
    ...
    result =
        writeFile(bankFn, (data + (params.romsize * bank)), params.romsize);
    if (result != 0) {
      goto Cleanup;
    }
...
int combineBios() {
  ...
  for (i = 0; i < MAX_BANKS; i++) {
    if (params.bank_files[i] == nullptr) continue;
...
int listBios() {
  ...
  if (isFlagSet(SW_LS_NV2A_TBL)) {
    ...
    int valid = (bios.available_space >= 0 &&
                 bios.available_space <= (int)bios.params.romsize);
...
int replicateBios() {
  ...
  // did user type romsize instead? (romsize param isnt used in command so free
  // to use either).
...
int decodeXcodes() {
  ...
  printf(
      "init tbl file: %s\nxcode count: %d\nxcode size: %d bytes\nxcode base: "
      "0x%x\n\n",
      params.in_file, context->xcodeCount, context->xcodeSize,
      context->xcodeBase);
...
int simulateXcodes() {
  ...
  while (interp.interpretNext(xcode) == 0) {
    ...
    if (xcode->opcode != XC_MEM_WRITE) continue;
    ...
    printf("\t%04lx: %s 0x%02x, 0x%08X\n",
           (unsigned long)(base + interp.offset - sizeof(XCODE)), opcode_str,
           xcode->addr, xcode->data);
  }
  ...
  // if -d flag is set, dump the memory to a file, otherwise print the memory
  // dump
  if (isFlagSet(SW_DMP)) {
    const char* filename = params.out_file;
    if (filename == nullptr) filename = "mem_sim.bin";
...
int encodeX86() {
  ...
  result =
      encodeX86AsMemWrites(data, dataSize, params.base, buffer, &xcodeSize);
  if (result != 0) {
...
int decompressFile() {
  ...
  printf("Decompressed %u -> %u bytes (%.3f%% compression)\n", dataSize,
         decompressedSize, savings);

  result =
      writeFileF(params.out_file, "decompressed file", buff, decompressedSize);
...
int help() {
  ...
  if (isFlagSet(SW_HELP)) {
    switch (cmd->type) {
      case CMD_LIST_BIOS:
        printf("# %s\n\n %s (req) *inferred\n %s\n %s\n %s\n %s\n\n",
               HELP_STR_LIST, HELP_STR_PARAM_IN_BIOS_FILE,
               HELP_STR_PARAM_LS_DATA_TBL, HELP_STR_PARAM_LS_NV2A_TBL,
               HELP_STR_PARAM_LS_DUMP_KRNL, HELP_STR_PARAM_LS_KEYS);
        printf("Usage: xbios -ls <bios_path> [switches]\n");
        return 0;

      case CMD_EXTRACT_BIOS:
        printf("# %s\n\n %s (req) *inferred\n %s\n %s\n %s\n\n",
               HELP_STR_EXTR_ALL, HELP_STR_PARAM_IN_BIOS_FILE,
               HELP_STR_PARAM_EXTRACT_KEYS, HELP_STR_PARAM_RESTORE_BOOT_PARAMS,
               HELP_STR_PARAM_WDIR);
      ...
      case CMD_COMBINE_BIOS:
        ...
        printf(
            "Usage: xbios -combine <bios_path1> <bios_path2> .. [switches]\n");
        return 0;

      case CMD_SIMULATE_XCODE:
        printf(
            "# %s\n\n %s (req) *inferred\n %s\n %s\n -d\t\t  - write sim to a "
            "file. Use -out to set output.\n\n",
      ...
      case CMD_DECODE_XCODE: {
        if (isFlagSet(SW_INI_FILE)) {
          ...
          printf(
              "# %s\n\n %s (req) *inferred\n %s\n %s\n %s\n -d\t\t  - write "
              "xcodes to a file. Use -out to set output\n\n",
              HELP_STR_XCODE_DECODE, HELP_STR_PARAM_IN_FILE,
              HELP_STR_PARAM_DECODE_INI, HELP_STR_PARAM_BASE,
              HELP_STR_PARAM_BRANCH);
          printf(
              "Use -xcode-decode -? -ini to get a list of decode "
              "settings\n\nUsage: xbios -xcode-decode <bios_path> "
              "[switches]\n");
        }
      }
        return 0;
      ...
      case CMD_ENCODE_X86:
        printf("# %s\n\n %s (req) *inferred\n %s\n\n", HELP_STR_X86_ENCODE,
               HELP_STR_PARAM_IN_FILE, HELP_STR_PARAM_OUT_FILE);
        printf("Usage: xbios -x86-encode <path> [switches]\n");
        return 0;

      case CMD_DUMP_PE_IMG:
        printf("# %s\n\n %s (req) *inferred\n\n", HELP_STR_DUMP_NT_IMG,
               HELP_STR_PARAM_IN_FILE);
      ...
      case CMD_COMPRESS_FILE:
        printf("# %s\n\n %s (req) *inferred\n %s (req)\n\n",
               HELP_STR_COMPRESS_FILE, HELP_STR_PARAM_IN_FILE,
               HELP_STR_PARAM_OUT_FILE);
        printf("Usage: xbios -compress <path> [switches]\n");
        return 0;

      case CMD_DECOMPRESS_FILE:
        printf("# %s\n\n %s (req) *inferred\n %s (req)\n\n",
               HELP_STR_DECOMPRESS_FILE, HELP_STR_PARAM_IN_FILE,
               HELP_STR_PARAM_OUT_FILE);
      ...
      case CMD_REPLICATE_BIOS:
        printf("# %s\n\n %s (req) *inferred\n %s (req) %s\n %s\n\n",
               HELP_STR_REPLICATE, HELP_STR_PARAM_IN_BIOS_FILE,
               HELP_STR_PARAM_BINSIZE, HELP_STR_VALID_ROM_SIZES,
               HELP_STR_PARAM_OUT_FILE);
        return 0;
...
int helpEncryption() {
  printf(
      ...
      "Use one of the switches below to specify a key for 2BL encryption / "
      "decryption.\n"
      "Not providing a switch results in 2BL not being encrypted / "
      "decrypted\n\n");
  ...
  // kernel
  printf(
      "\n\nKernel encryption / decryption:\nOnly needed for custom BIOSes as "
...
int inject_xcodes(uint8_t* data, uint32_t size, uint8_t* xcodes,
                  uint32_t xcodesSize) {
  int result;
  XcodeInterp interp;
  ...
  if (init_tbl->data_tbl_offset != 0) {
    if (init_tbl->data_tbl_offset < 0x80 + interp.offset + xcodesSize) {
      // printf("no space for xcodes. only %d bytes available. xcode size: %d
      // bytes\n", (0x80 + interp.offset + xcodesSize -
      // init_tbl->data_tbl_offset), xcodesSize);
  ...
  if (jmp) {
    printf(
        "XCODE: replacing quit xcode at 0x%lx with jump to free space at "
        "0x%x\n",
...
int read_keys() {
  ...
  if (params.bldr_key_file != nullptr) {
    ...
    if (params.bldr_key == nullptr) return 1;
    printf("bldr key: ");
  ...
  if (params.kernel_key_file != nullptr) {
    ...
    if (params.kernel_key == nullptr) return 1;
    printf("krnl key: ");
    uprinth(params.kernel_key, XB_KEY_SIZE);
  }

  if (params.bldr_key != nullptr || params.kernel_key != nullptr) printf("\n");
...
int read_mcpx() {
  ...
  if (params.mcpx_file == nullptr) return 0;

  mcpxData = readFile(params.mcpx_file, nullptr, MCPX_BLOCK_SIZE);
  if (mcpxData == nullptr) return 1;
...
uint8_t* load_init_tbl_file(uint32_t* size, uint32_t* base) {
  ...
  if (init_tbl == nullptr) return nullptr;
...
void printBldrInfo(Bios* bios) {
  ...
  uprintc((init_tbl_size >= 0 && init_tbl_size <= romsize), "%u",
          init_tbl_size);
  printf(" bytes\n");
  ...
  uprintc((kernel_data_size >= 0 && kernel_data_size <= romsize), "%u",
          kernel_data_size);
  printf(" bytes\n");
  printf("\n");
}
...
void printPreldrInfo(Bios* bios) {
  ...
  if (bios->preldr.status > PRELDR_STATUS_FOUND) return;
  ...
  if (jmp_address < PRELDR_REAL_BASE ||
      jmp_address > PRELDR_REAL_BASE + PRELDR_BLOCK_SIZE) {
    // escaped mcpx!
  }
...
void printNv2aInfo(Bios* bios) {
  ...
  const char* init_tbl_comments[5] = {
      "; init tbl revision",   "", "", "; kernel version + init tbl identifier",
      "; rom data tbl offset",
  };
  ...
  for (i = 0; i < ARRAY_SIZE; ++i) {
    ...
    if (cHasValue && hasValue)
      snprintf(str + i * 3, sizeof(str) - (i * 3), "%02lX ",
               (unsigned long)valArray[i]);
  }
  if (hasValue) {
    if (str[0] == '\0') {
      i = 0;
  ...
  printf("%04x-%04x:\t%s\n", ARRAY_START * UINT_ALIGN,
         (ARRAY_START * UINT_ALIGN) + (ARRAY_SIZE * UINT_ALIGN) - UINT_ALIGN,
         str);
  ...
  for (i = ARRAY_START + ARRAY_SIZE; i < sizeof(INIT_TBL) / UINT_ALIGN; ++i) {
    item = ((uint32_t*)init_tbl)[i];
    printf("%04x:\t\t0x%08x %s\n", i * UINT_ALIGN, item,
           init_tbl_comments[i - ARRAY_START - ARRAY_SIZE]);
  }
  printf("\n");
}
...
void printDataTblInfo(Bios* bios) {
  ...
  ROM_DATA_TBL* data_tbl =
      (ROM_DATA_TBL*)(bios->data + bios->init_tbl->data_tbl_offset);

  static const char* padTblName[5] = {"Fastest", "Fast", "Standard", "Slow",
                                      "Slowest"};
  ...
  printf(
      ...
      data_tbl->cal.max_m_clk, data_tbl->cal.slowest.countA,
      data_tbl->cal.slowest.countB, data_tbl->cal.slow.countA,
      data_tbl->cal.slow.countB, data_tbl->cal.standard.countA,
      data_tbl->cal.standard.countB, data_tbl->cal.fast.countA,
      data_tbl->cal.fast.countB, data_tbl->cal.fastest.countA,
      data_tbl->cal.fastest.countB);
  ...
  for (int i = 0; i < 5; i++) {
    printf(
        ...
        padTblName[i], data_tbl->samsung[i].adr_drv_fall,
        data_tbl->samsung[i].adr_drv_rise, data_tbl->micron[i].adr_drv_fall,
        data_tbl->micron[i].adr_drv_rise, data_tbl->samsung[i].adr_slw_fall,
        data_tbl->samsung[i].adr_slw_rise, data_tbl->micron[i].adr_slw_fall,
        data_tbl->micron[i].adr_slw_rise, data_tbl->samsung[i].clk_drv_fall,
        data_tbl->samsung[i].clk_drv_rise, data_tbl->micron[i].clk_drv_fall,
        data_tbl->micron[i].clk_drv_rise, data_tbl->samsung[i].clk_slw_fall,
        data_tbl->samsung[i].clk_slw_rise, data_tbl->micron[i].clk_slw_fall,
        data_tbl->micron[i].clk_slw_rise, data_tbl->samsung[i].dat_drv_fall,
        data_tbl->samsung[i].dat_drv_rise, data_tbl->micron[i].dat_drv_fall,
        data_tbl->micron[i].dat_drv_rise, data_tbl->samsung[i].dat_slw_fall,
        data_tbl->samsung[i].dat_slw_rise, data_tbl->micron[i].dat_slw_fall,
        data_tbl->micron[i].dat_slw_rise, data_tbl->samsung[i].dqs_drv_fall,
        data_tbl->samsung[i].dqs_drv_rise, data_tbl->micron[i].dqs_drv_fall,
        data_tbl->micron[i].dqs_drv_rise, data_tbl->samsung[i].dqs_slw_fall,
        data_tbl->samsung[i].dqs_slw_rise, data_tbl->micron[i].dqs_slw_fall,
        data_tbl->micron[i].dqs_slw_rise, data_tbl->samsung[i].dat_inb_delay,
        data_tbl->micron[i].dat_inb_delay, data_tbl->samsung[i].clk_ic_delay,
        data_tbl->micron[i].clk_ic_delay, data_tbl->samsung[i].dqs_inb_delay,
        data_tbl->micron[i].dqs_inb_delay);
  }
  printf("\n");
}
...
void printKeyInfo(Bios* bios) {
  ...
  if (bios->kernel.img != nullptr) {
    if (rsa_findPublicKey(bios->kernel.img, bios->kernel.img_size, &pubkey,
                          nullptr) == RSA_ERROR_SUCCESS) {
      printf("\nPublic key:\b\b\b\b");
      uprinthl((uint8_t*)&pubkey->modulus, RSA_MOD_SIZE(&pubkey->header), 16,
               "\t\t", 0);
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  result = parseCli(argc, argv, cmd, cmd_tbl, sizeof(cmd_tbl), param_tbl,
                    sizeof(param_tbl));
  if (result != 0) {
    switch (result) {
  ...
  if (read_keys() != 0) goto Exit;

  if (read_mcpx() != 0) goto Exit;
