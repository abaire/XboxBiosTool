// loadini.c: implements the loadini functions for loading settings from a
// settings file.
...
int loadini(FILE* stream, const LOADINI_SETTING_MAP* settings_map,
  ...
  while (fgets(buf, LOADINI_MAX_LINE_SIZE, stream) != NULL) {
    ...
    for (i = 0; i < map_size / sizeof(LOADINI_SETTING_MAP); i++) {
      if (strcmp(key, settings_map[i].s->key) != 0) continue;
...
static void set_setting_value(char** setting, const char* value, uint32_t len) {
  if (setting == NULL || *setting != NULL) return;
