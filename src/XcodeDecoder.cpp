...
#define XC_WRITE_MASK(i, xc_cmd, xc_addr, addr_mask, xc_data, data_mask) \
  (((uint8_t*)(_ptr + i)) >= _data) &&                                   \
      ((_ptr + i)->opcode == xc_cmd &&                                   \
       ((_ptr + i)->addr & addr_mask) == xc_addr &&                      \
       IS_NEG_TRUE(((_ptr + i)->data & data_mask), xc_data))

#define XC_WRITE(i, xc_cmd, xc_addr, xc_data) \
  XC_WRITE_MASK(i, xc_cmd, xc_addr, 0xFFFFFFFF, xc_data, 0xFFFFFFFF)
...
#define XC_WRITE_COMMENT(comment, xc_cmd, xc_addr, xc_data) \
  XC_WRITE_STATEMENT(xc_cmd, xc_addr, xc_data, strcat(str, comment))

#define XC_WRITE_COMMENT_LAST_XCODE(comment, xc_cmd, xc_addr, xc_data,       \
                                    xc_prev_cmd, xc_prev_addr, xc_prev_data) \
  if (XC_WRITE(0, xc_cmd, xc_addr, xc_data) &&                               \
...
#define XC_WRITE_COMMENT_NEXT_XCODE(comment, xc_cmd, xc_addr, xc_data,       \
                                    xc_prev_cmd, xc_prev_addr, xc_prev_data) \
  if (XC_WRITE(0, xc_cmd, xc_addr, xc_data) &&                               \
      XC_WRITE(1, xc_prev_cmd, xc_prev_addr, xc_prev_data)) {                \
    strcat(str, comment);                                                    \
...
#define XC_WRITE_COMMENT_LAST_2_XCODE(                            \
    comment, xc_cmd, xc_addr, xc_data, xc_prev_cmd, xc_prev_addr, \
    xc_prev_data, xc_prev1_cmd, xc_prev1_addr, xc_prev1_data)     \
  if (XC_WRITE(0, xc_cmd, xc_addr, xc_data) &&                    \
      XC_WRITE(-1, xc_prev_cmd, xc_prev_addr, xc_prev_data) &&    \
...
#define XC_WRITE_COMMENT_NEXT_2_XCODE(                            \
    comment, xc_cmd, xc_addr, xc_data, xc_prev_cmd, xc_prev_addr, \
    xc_prev_data, xc_prev1_cmd, xc_prev1_addr, xc_prev1_data)     \
  if (XC_WRITE(0, xc_cmd, xc_addr, xc_data) &&                    \
      XC_WRITE(1, xc_prev_cmd, xc_prev_addr, xc_prev_data) &&     \
...
#define XC_WRITE_COMMENT_MASK_LAST_XCODE(                                     \
    comment, xc_cmd, xc_addr, xc_addr_mask, xc_data, xc_prev_cmd,             \
    xc_prev_addr, xc_prev_addr_mask, xc_prev_data)                            \
  if (XC_WRITE_MASK(0, xc_cmd, xc_addr, xc_addr_mask, xc_data, 0xFFFFFFFF) && \
      XC_WRITE_MASK(-1, xc_prev_cmd, xc_prev_addr, xc_prev_addr_mask,         \
                    xc_prev_data, 0xFFFFFFFF)) {                              \
    strcat(str, comment);                                                     \
    return 0;                                                                 \
  }

#define XC_WRITE_COMMENT_MASK_NEXT_XCODE(                                    \
    comment, xc_cmd, xc_addr, xc_addr_mask, xc_data, xc_prev_cmd,            \
    xc_prev_addr, xc_prev_addr_mask, xc_prev_data)                           \
  if (XC_WRITE_COMMENT_MASK(0, xc_cmd, xc_addr, xc_addr_mask, xc_data) &&    \
      XC_WRITE_COMMENT_MASK(1, xc_prev_cmd, xc_prev_addr, xc_prev_addr_mask, \
                            xc_prev_data)) {                                 \
    strcat(str, comment);                                                    \
    return 0;                                                                \
  }
...
static const DECODE_SETTING_MAP num_str_fields[] = {
    {"{hex}", "%x"}, {"{hex8}", "%08x"}, {"{HEX}", "%X"}, {"{HEX8}", "%08X"}};
static const FIELD_MAP format_map[] = {{"{offset}", DECODE_FIELD_OFFSET},
...
const LOADINI_RETURN_MAP decode_settings_map = {
    settings_map, sizeof(settings_map),
    sizeof(settings_map) / sizeof(LOADINI_SETTING_MAP)};

static int ll(char* output, char* str, uint32_t i, uint32_t* j, uint32_t len,
              uint32_t m);
...
int XcodeDecoder::load(uint8_t* data, uint32_t size, uint32_t base,
                       const char* ini) {
...
int XcodeDecoder::loadSettings(const char* ini,
  ...
  static const char* default_format_str =
  ...
  for (i = 0; i < len; i++) {
    ...
    for (j = 0;
         j < sizeof(settings->format_map) / sizeof(DECODE_STR_SETTING_MAP);
         j++) {
  ...
  if (settings->num_str != NULL) {
    ...
    for (i = 0; i < len; ++i) {
      ...
      for (j = 0; j < sizeof(num_str_fields) / sizeof(DECODE_SETTING_MAP);
           ++j) {
        if (strcmp(buf, num_str_fields[j].field) == 0) {
          ...
          snprintf(settings->num_str_format, buf_size, settings->num_str,
                   num_str_fields[j].value);
          break;
        }
      }
  ...
  while (interp.interpretNext(context->xcode) == 0) {
    ...
    if (result != 0) {
      if (result == ERROR_BUFFER_OVERFLOW) {
        ...
        printf(
            "Error: decoding xcode:\n\t%04lX, OP: %02X, ADDR: %04X, DATA: "
            "%04X\n",
            (unsigned long)(context->xcodeBase + interp.offset - sizeof(XCODE)),
            context->xcode->opcode, context->xcode->addr, context->xcode->data);
      }
      return result;
    }
  }
...
int XcodeDecoder::decode() {
  ...
  if (searchLabel(context, interp.offset - sizeof(XCODE), &label) == 0) {
    ...
    if (context->settings.label_on_new_line) strcat(str, "\n");
  }
  ...
  for (uint32_t seq = 0; seq < sizeof(context->settings.format_map) /
                                   sizeof(DECODE_STR_SETTING_MAP);
       seq++) {
    for (uint32_t j = 0; j < sizeof(context->settings.format_map) /
                                 sizeof(DECODE_STR_SETTING_MAP);
         j++) {
      if (context->settings.format_map[j].seq != seq + 1) continue;
      if (context->settings.format_map[j].str == NULL) break;
      ...
      jmp_len = context->settings.labelMaxLen +
                strlen(context->settings.jmp_str) - 3 + fmt_len + 1;
      ...
      switch (context->settings.format_map[j].type) {
        ...
        case DECODE_FIELD_OFFSET: {
          snprintf(str_tmp, sizeof(str_tmp),
                   context->settings.format_map[j].str, "%04x");
          snprintf(str, sizeof(str), str_tmp,
                   context->xcodeBase + interp.offset - sizeof(XCODE));
        } break;

        // output OPCODE
        case DECODE_FIELD_OPCODE: {
          const char* str_opcode;
          if (getOpcodeStr(context->settings.opcodes, context->xcode->opcode,
                           str_opcode) != 0)
            return 1;
          snprintf(str, sizeof(str), context->settings.format_map[j].str,
                   str_opcode);
        ...
        case DECODE_FIELD_ADDRESS: {
          switch (context->xcode->opcode) {
            case XC_JMP: {
              ...
              if (searchLabel(context, interp.offset + context->xcode->data,
                              &label) == 0) {
                snprintf(str_tmp, sizeof(str_tmp), context->settings.jmp_str,
                         label->name);
              }
            } break;
            ...
            case XC_USE_RESULT: {
              if (context->settings.opcode_use_result) {
                ...
                if (getOpcodeStr(context->settings.opcodes,
                                 (uint8_t)context->xcode->addr,
                                 opcode_str) == 0) {
            ...
            default: {
              snprintf(str_tmp, sizeof(str_tmp),
                       context->settings.num_str_format, context->xcode->addr);
              break;
            }
          }
          ...
          if (context->settings.pad) {
            ...
            if (operand_len < no_operand_len) operand_len = no_operand_len;
        ...
        case DECODE_FIELD_DATA: {
          switch (context->xcode->opcode) {
            ...
            default:
              snprintf(str_tmp, sizeof(str_tmp),
                       context->settings.num_str_format, context->xcode->data);
              break;
          }
          ...
          snprintf(str, sizeof(str), context->settings.format_map[j].str,
                   str_tmp);
          if (context->settings.pad) {
            if (operand_len < jmp_len) operand_len = jmp_len;
            if (operand_len < no_operand_len) operand_len = no_operand_len;
            rpad(str, operand_len, ' ');
          }
        } break;
...
int XcodeDecoder::getCommentStr(char* str, size_t buf_size) {
  ...
  XC_WRITE_COMMENT("smbus read revision register", XC_IO_WRITE,
                   SMB_CMD_REGISTER, 0x01);

  // CX871
  XC_WRITE_COMMENT("CX871 slave address", XC_IO_WRITE, SMB_BASE + 0x04, 0x8A);
  XC_WRITE_COMMENT_NEXT_XCODE("CX871 0xBA = 0x3F", XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0xBA, XC_IO_WRITE,
                              SMB_VAL_REGISTER, 0x3F);
  XC_WRITE_COMMENT_NEXT_XCODE("CX871 0x6C = 0x46", XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0x6C, XC_IO_WRITE,
                              SMB_VAL_REGISTER, 0x46);
  XC_WRITE_COMMENT_NEXT_XCODE("CX871 0xB8 = 0x00", XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0xB8, XC_IO_WRITE,
                              SMB_VAL_REGISTER, 0x00);
  XC_WRITE_COMMENT_NEXT_XCODE("CX871 0xCE = 0x19", XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0xCE, XC_IO_WRITE,
                              SMB_VAL_REGISTER, 0x19);
  XC_WRITE_COMMENT_NEXT_XCODE("CX871 0xC6 = 0x9C", XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0xC6, XC_IO_WRITE,
                              SMB_VAL_REGISTER, 0x9C);
  XC_WRITE_COMMENT_NEXT_XCODE("CX871 0x32 = 0x08", XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0x32, XC_IO_WRITE,
                              SMB_VAL_REGISTER, 0x08);
  XC_WRITE_COMMENT_NEXT_XCODE("CX871 0xC4 = 0x01", XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0xC4, XC_IO_WRITE,
                              SMB_VAL_REGISTER, 0x01);

  // focus
  XC_WRITE_COMMENT("focus slave address", XC_IO_WRITE, SMB_BASE + 0x04, 0xD4);

  // xcalibur
  XC_WRITE_COMMENT("xcalibur slave address", XC_IO_WRITE, SMB_BASE + 0x04,
                   0xE1);

  XC_WRITE_COMMENT_NEXT_XCODE("report memory type", XC_IO_WRITE,
                              SMB_BASE + 0x04, 0x20, XC_IO_WRITE,
                              SMB_CMD_REGISTER, 0x13);
  ...
  XC_WRITE_COMMENT("smc slave write address", XC_IO_WRITE, SMB_BASE + 0x04,
                   0x20);
  XC_WRITE_COMMENT("smc slave read address", XC_IO_WRITE, SMB_BASE + 0x04,
                   0x21);

  XC_WRITE_COMMENT("smbus kickoff", XC_IO_WRITE, SMB_BASE + 0x02, 0x0A);

  // mcpx v1.0 io bar
  XC_WRITE_COMMENT("read io bar (B02) MCPX v1.0", XC_PCI_READ, MCPX_1_0_IO_BAR,
                   0);
  XC_WRITE_COMMENT("set io bar (B02) MCPX v1.0", XC_PCI_WRITE, MCPX_1_0_IO_BAR,
                   MCPX_IO_BAR_VAL);
  XC_WRITE_COMMENT_LAST_XCODE("jmp if (B02) MCPX v1.0", XC_JNE, MCPX_IO_BAR_VAL,
                              -1, XC_PCI_READ, MCPX_1_1_IO_BAR, 0);

  // mcpx v1.1 io bar
  XC_WRITE_COMMENT("read io bar (C03) MCPX v1.1", XC_PCI_READ, MCPX_1_1_IO_BAR,
                   0);
  XC_WRITE_COMMENT("set io bar (C03) MCPX v1.1", XC_PCI_WRITE, MCPX_1_1_IO_BAR,
                   MCPX_IO_BAR_VAL);
  XC_WRITE_COMMENT_LAST_XCODE("jmp if (C03) MCPX v1.1", XC_JNE, MCPX_IO_BAR_VAL,
                              -1, XC_PCI_READ, MCPX_1_0_IO_BAR, 0);

  // spin loop
  XC_WRITE_COMMENT_LAST_XCODE("spin until smbus is ready", XC_JNE, 0x10, -18,
  ...
  XC_WRITE_COMMENT_MASK("turn off secret rom", XC_PCI_WRITE, 0x80000880,
                        0xF000FFFF, 0x2);
  ...
  XC_WRITE_COMMENT("smbus is bad, flatline clks", XC_PCI_WRITE, 0x8000036C,
                   0x1000000);
  ...
  // nv clk
  XC_WRITE_COMMENT("set nv clk 155 MHz", XC_MEM_WRITE, NV2A_BASE + NV_CLK_REG,
                   0x11701);

  size_t str_len = strlen(str);
  XC_WRITE_STATEMENT(
      XC_MEM_WRITE, NV2A_BASE + NV_CLK_REG, -1, uint32_t base = 16667;
      uint32_t nvclk = base * ((_ptr->data & 0xFF00) >> 8);
      nvclk /= 1 << ((_ptr->data & 0x70000) >> 16); nvclk /= _ptr->data & 0xFF;
      nvclk /= 1000; snprintf(str + str_len, buf_size - str_len,
                              "set nv clk %dMHz (@ %.3fMHz)", nvclk,
                              (float)(base / 1000.00f)));

  // nv gpu revision
  XC_WRITE_COMMENT_NEXT_XCODE("get nv rev", XC_MEM_READ, NV2A_BASE, 0,
                              XC_AND_OR, 0xFF, 0);

  XC_WRITE_COMMENT_LAST_2_XCODE("if nv rev != A2", XC_JNE, 0xA1, -1, XC_AND_OR,
                                0xFF, 0, XC_MEM_READ, NV2A_BASE, 0);

  XC_WRITE_COMMENT_LAST_2_XCODE("if nv rev != A1", XC_JNE, 0xA2, -1, XC_AND_OR,
                                0xFF, 0, XC_MEM_READ, NV2A_BASE, 0);

  // ROM pad
  XC_WRITE_COMMENT_NEXT_XCODE("configure pad for micron", XC_MEM_WRITE,
                              NV2A_BASE + 0x1214, 0x28282828, XC_MEM_WRITE,
                              NV2A_BASE + 0x122C, 0x88888888);

  XC_WRITE_COMMENT_NEXT_XCODE("configure pad for samsung", XC_MEM_WRITE,
                              NV2A_BASE + 0x1214, 0x09090909, XC_MEM_WRITE,
                              NV2A_BASE + 0x122C, 0xAAAAAAAA);

  XC_WRITE_COMMENT_NEXT_2_XCODE("memory pad configuration", XC_MEM_WRITE,
                                NV2A_BASE + 0x1230, 0xFFFFFFFF, XC_MEM_WRITE,
                                NV2A_BASE + 0x1234, 0xAAAAAAAA, XC_MEM_WRITE,
                                NV2A_BASE + 0x1238, 0xAAAAAAAA);

  str_len = strlen(str);
  XC_WRITE_STATEMENT(
      XC_PCI_WRITE, 0x80000084, -1,
      snprintf(str + str_len, buf_size - str_len, "set memory size %d Mb\n",
               (_ptr->data + 1) / 1024 / 1024));

  XC_WRITE_COMMENT("set extbank bit (00000F00)", XC_MEM_WRITE,
                   NV2A_BASE + 0x100200, 0x03070103);
  XC_WRITE_COMMENT("clear extbank bit (00000F00)", XC_MEM_WRITE,
                   NV2A_BASE + 0x100200, 0x03070003);

  XC_WRITE_COMMENT("clear scratch pad (mem type)", XC_PCI_WRITE, 0x8000103C, 0);
  XC_WRITE_COMMENT("clear scratch pad (mem result)", XC_PCI_WRITE, 0x8000183C,
                   0);

  XC_WRITE_COMMENT_MASK("mem test pattern 1", XC_MEM_WRITE, 0x00555508,
                        0x00FFFF0F, MEMTEST_PATTERN1);
  XC_WRITE_COMMENT_MASK("mem test pattern 2", XC_MEM_WRITE, 0x00555508,
                        0x00FFFF0F, MEMTEST_PATTERN2);
  XC_WRITE_COMMENT_MASK("mem test pattern 3", XC_MEM_WRITE, 0x00555508,
                        0x00FFFF0F, MEMTEST_PATTERN3);

  XC_WRITE_COMMENT_MASK_LAST_XCODE("does dram exist?", XC_JNE, MEMTEST_PATTERN2,
                                   0xFFFFFFFF, -1, XC_MEM_READ, 0x00555508,
                                   0x00FFFF0F, 0);

  XC_WRITE_COMMENT_NEXT_XCODE("15ns delay by performing jmps", XC_JMP, 0, 0,
                              XC_JMP, 0, 0);

  XC_WRITE_COMMENT_LAST_2_XCODE("don't gen INIT# on powercycle", XC_USE_RESULT,
                                0x04, MCPX_LEG_24, XC_AND_OR, 0xFFFFFFFF, 0x400,
  ...
  XC_WRITE_COMMENT("set ctrim2 ( samsung )", XC_MEM_WRITE, 0x0f0010b8,
                   0xFFFF0000);
  XC_WRITE_COMMENT("set ctrim2 ( micron )", XC_MEM_WRITE, 0x0f0010b8,
                   0xEEEE0000);
...
static void walkBranch(DECODE_CONTEXT* context, XcodeInterp* interp) {
  ...
  while (interp->interpretNext(xc) == 0) {
    if (interp->offset - sizeof(XCODE) == endOffset) break;
