...
int rsa_verifyPublicKey(uint8_t* data, uint32_t size, uint32_t offset,
                        PUBLIC_KEY** pubkey) {
  ...
  if (sizeof(RSA_HEADER) > size ||
      offset + sizeof(RSA_HEADER) > size)  // header over run
  ...
  if (rsa_header->mod_size == 0 || rsa_header->bits == 0 ||
      rsa_header->max_bytes == 0 || rsa_header->exponent == 0)
    return RSA_ERROR;

  if (rsa_header->bits % 8) return RSA_ERROR;

  if (RSA_MOD_SIZE(rsa_header) > rsa_header->mod_size) return RSA_ERROR;

  if (memcmp(rsa_header->magic, &RSA1_MAGIC, 4) != 0)  // no match
    return RSA_ERROR;

  if (pubkey != NULL) *pubkey = (PUBLIC_KEY*)(rsa_header);
...
int rsa_findPublicKey(uint8_t* data, uint32_t size, PUBLIC_KEY** pubkey,
  ...
  size_t i;
  int result = 0;
  if (data == NULL || pubkey == NULL) return RSA_ERROR_INVALID_DATA;

  for (i = 0; i < size - sizeof(RSA_HEADER); ++i) {
    result = rsa_verifyPublicKey(data, size, i, pubkey);
    if (result != RSA_ERROR) break;
  }
