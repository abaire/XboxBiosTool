// cli_tbl.cpp: Implements functions for parsing command line arguments based on
// a command and parameter table.
...
int getCmd(const CMD_TBL* cmds, const int cmd_size, const char* arg,
           const CMD_TBL** cmd) {
...
int parseCli(int argc, char* argv[], const CMD_TBL*& cmd, const CMD_TBL* cmds,
             const int cmd_size, const PARAM_TBL* params,
  ...
  if (argc == 1) return CLI_ERROR_NO_CMD;
  if (strlen(argv[1]) < 2) return CLI_ERROR_INVALID_CMD;
  ...
  for (i = startIndex; i < argc; i++) {
    ...
    if (argv[i][0] != '-' && argv[i][0] != '/') {
      ...
      printf(
          "Error: No corresponding switch defined for argument, "
          "'%s'.\n\nUsage: -<switch> <value>\n",
          argv[i]);
    ...
    for (j = 0; j < (int)(param_size / sizeof(PARAM_TBL)); j++) {
      if (strcmp(arg, params[j].sw) != 0) continue;

      setFlag(params[j].swType);

      if (isFlagClear(SW_HELP)) {
        swNeedValue = (params[j].cmdType != PARAM_TBL::BOOL) &&
                      (params[j].cmdType != PARAM_TBL::FLAG);
        if (swNeedValue && i + 1 >= argc) {
          printf(
              "Error: '-%s' switch is missing an argument\n\nUsage: -%s "
              "<value>\n",
              arg, arg);
        ...
        if (swNeedValue && (argv[i + 1][0] == '-' || argv[i + 1][0] == '/')) {
          printf(
              "Error: '-%s' switch has an invaild argument, '%s'\n\nUsage: -%s "
              "<value>\n",
              arg, argv[i + 1], arg);
          return CLI_ERROR_INVALID_ARG;
        }
  ...
  for (i = 0; i < (int)(param_size / sizeof(PARAM_TBL)); i++) {
    for (j = 0;
         j < sizeof(cmd->requiredSwitches) / sizeof(cmd->requiredSwitches[0]);
         j++) {
      if (cmd->requiredSwitches[j] == 0) break;

      if (params[i].swType != cmd->requiredSwitches[j]) continue;
  ...
  if (missing) return CLI_ERROR_MISSING_SW;

  return 0;
}

void setParamValue(const PARAM_TBL* param, char* arg) {
  if (param->var == NULL) return;
