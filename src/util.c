// util.cpp: utility functions. printing to console, changing console color,
// etc.
...
void uprintf(char* data, const size_t size, const char* format, ...) {
  ...
  if (data == NULL || format == NULL || size < 1 || strlen(format) < 1) return;
  ...
  while (1) {
    ...
    if (*formatPtr == START_TOKEN) {
      ...
      while (*formatPtr != END_TOKEN) {
        if (*formatPtr == '\0') break;
        formatPtr++;
      }

      if (*formatPtr != END_TOKEN) break;
      ...
      int num = 0;
      int tokenLen =
          (endToken - startToken + 1) - 2;  // -2 to remove the '{' and '}'
      char tokenStr[10] = {0};
      ...
      if (*formatPtr != START_TOKEN) formatPtr += argLen;
    }

    if (*formatPtr != START_TOKEN) formatPtr++;
  }
