// Bios.cpp: Implements functions for loading an Original Xbox BIOS file,
// decrypting it, and extracting the bootloader, kernel, and other data.
...
static int validate_required_space(const uint32_t requiredSpace,
                                   uint32_t* size);

int Bios::load(uint8_t* buff, const uint32_t binsize,
...
int Bios::build(BIOS_BUILD_PARAMS* build_params, uint32_t binsize,
  ...
  const uint32_t requiredSpace =
      BLDR_BLOCK_SIZE + MCPX_BLOCK_SIZE + build_params->kernel_size +
      build_params->kernel_data_size + build_params->init_tbl_size;
  ...
  kernel.encryption_state = (!params.enc_kernel && params.kernel_key == NULL) ||
                            (params.enc_kernel && params.kernel_key != NULL);
...
int Bios::init(uint8_t* buff, const uint32_t binsize,
               const BIOS_LOAD_PARAMS* bios_params) {
  ...
  if (buff == NULL) {
    ...
    if (data == NULL) return BIOS_LOAD_STATUS_FAILED;
...
void Bios::getOffsets2() {
  ...
  uint32_t entry_offset =
      (bldr.ldr_params->bldr_entry_point & 0x0000FFFF /*- BLDR_BASE*/);
  ...
  if (IN_BOUNDS(bldr.entry, bldr.data, BLDR_BLOCK_SIZE) == false) {
    ...
    if (IN_BOUNDS_BLOCK(bldr.bfm_key, XB_KEY_SIZE, bldr.data,
                        BLDR_BLOCK_SIZE) == false) {
      bldr.bfm_key = NULL;
    }
  }

  // set the kernel pointers.
  kernel.uncompressed_data_ptr =
      (bldr.data - bldr.boot_params->uncompressed_kernel_data_size);
  kernel.compressed_kernel_ptr =
      (kernel.uncompressed_data_ptr - bldr.boot_params->compressed_kernel_size);

  // calculate the available space in the bios.
  available_space = params.romsize - MCPX_BLOCK_SIZE - BLDR_BLOCK_SIZE -
                    bldr.boot_params->uncompressed_kernel_data_size -
                    bldr.boot_params->compressed_kernel_size;
}
...
void Bios::preldrCreateKey(uint8_t* sbkey, uint8_t* key) {
  ...
  for (int i = 0; i < XB_KEY_SIZE; ++i) key[i] = sbkey[i] ^ 0x5C;
...
void Bios::preldrValidateAndDecryptBldr() {
  ...
  preldr.jmp_offset =
      preldr.params->jmp_offset + 5;  // +5 bytes (opcode + offset)

  if (preldr.params->jmp_opcode != 0xE9) {
    return;
  }

  preldr.ptr_block = (PRELDR_PTR_BLOCK*)(preldr.data + preldr.jmp_offset -
                                         sizeof(PRELDR_PTR_BLOCK));
  if (IN_BOUNDS(preldr.ptr_block, preldr.data, PRELDR_BLOCK_SIZE) == false) {
    ...
    // public key pointer
    preldr.public_key =
        (XB_PUBLIC_KEY*)(preldr.data + preldr.ptr_block->public_key_ptr -
                         PRELDR_REAL_BASE);
    ...
    preldr.func_block =
        (PRELDR_FUNC_BLOCK*)(preldr.data + preldr.ptr_block->func_block_ptr -
                             PRELDR_REAL_BASE);
  ...
  if (!bldr.encryption_state) return;

  preldrSymmetricEncDecBldr(
      preldr.bldr_key, SHA1_DIGEST_LEN); /*dont mangle the FBL section of 2BL.*/

  // calculate the offset to the 2BL entry point.
  PRELDR_ENTRY* entry =
      (PRELDR_ENTRY*)(bldr.data + BLDR_BLOCK_SIZE - PRELDR_BLOCK_SIZE -
                      sizeof(PRELDR_ENTRY));
  ...
  if (params.restore_boot_params) {
    ...
    bldr.boot_params =
        (BOOT_PARAMS*)((uint8_t*)bldr.boot_params - PRELDR_NONCE_SIZE);
  }
...
void Bios::preldrSymmetricEncDecBldr(const uint8_t* key, const uint32_t len) {
  ...
  rc4(&context, bldr.data + BLDR_BLOCK_SIZE - PRELDR_PARAMS_SIZE,
      PRELDR_PARAMS_SIZE);
...
void Bios::symmetricEncDecKernel() {
  ...
  if (key == NULL) {
    if (bldr.keys == NULL) return;

    key = bldr.keys->kernel_key;
    if (key == NULL) return;

    uint8_t tmp[XB_KEY_SIZE] = {0};
    if (memcmp(key, tmp, XB_KEY_SIZE) == 0) return;  // kernel key is all 0's

    memset(tmp, 0xFF, XB_KEY_SIZE);
    if (memcmp(key, tmp, XB_KEY_SIZE) == 0) return;  // kernel key is all FF's
  ...
  rc4(&context, kernel.compressed_kernel_ptr,
      bldr.boot_params->compressed_kernel_size);
...
int Bios::decompressKrnl() {
  ...
  if (kernel.compressed_kernel_ptr == NULL ||
      bios_status != BIOS_LOAD_STATUS_SUCCESS) {
    return 1;
  }
  ...
  if (kernel.img == NULL) return 1;
  if (lzx_decompress(kernel.compressed_kernel_ptr,
                     bldr.boot_params->compressed_kernel_size, &kernel.img,
                     &buffer_size, &kernel.img_size) != 0)
    return 1;
  return 0;
}
...
int Bios::preldrDecryptPublicKey() {
  ...
  if (preldr.public_key == NULL) return 1;
...
int bios_replicate_data(uint32_t from, uint32_t to, uint8_t* buffer,
  ...
  if (from >= to || buffersize < to) return 1;
